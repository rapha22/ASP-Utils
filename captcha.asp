...
'!!! WARNING !!! --------------------------------------------------------------------------------------------------
'In VBScript, it's not possible to use ReDim to dynamically create a multidimensional array, that's why Execute was used. Be careful!
If Request("width") <> "" And Request("height") <> "" Then
	Execute("Dim Bitmap(" & Request("height") & ", " & Request("width") & ")") '[Height,Width]
Else
	Execute("Dim Bitmap(22, 91)") '[Height,Width]
End If
'------------------------------------------------------------------------------------------------------------------


Dim CodeLength : CodeLength = 6 'Secure code length (Max:8)
		
If Request("length") <> "" Then CodeLength = CInt(Request("length"))
